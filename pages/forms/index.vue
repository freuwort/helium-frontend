<template>
    <NuxtLayout name="auth-default" :scope :pageTitle="IPM.options.pageTitle" color="#06B6D4">
        <HeCard>
            ...
            >
                <template #right>
                    <IodButton type="button" label="Neues Formular" corner="pill" icon-right="add" @click="IPM.open()"/>
                </template>
            </IodTable>
        </HeCard>
    </NuxtLayout>
...
<script lang="ts" setup>
    ...
    const tableColumns = [
        ...
        { name: 'form_submissions_count', label: 'Einsendungen', valuePath: 'form_submissions_count', sortable: false, width: 200, resizable: true, hideable: true, default: '-', transform: (value: number | null) => (value || 0).toString() },
